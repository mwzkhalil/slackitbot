...
class ELazarAgent:
    ...
    async def respond(self, query: str, client_id: Optional[str] = None) -> str:
        ...
        if any(identity_q in query_lower for identity_q in identity_questions):
            return (
                "I am E. Lezzat, your HR and Internal Operations assistant. I help answer "
                "questions about HR policies, internal workflows, SOPs, holidays, passwords, "
                "system access, performance reviews, and other internal procedures. I can access "
                "our internal documentation to provide you with accurate, step-by-step answers "
                "to your questions."
            )
        ...
        if drive_ids:
            ...
            # Check first document preview to verify it's a Drive doc
            if drive_docs:
        ...
        if drive_docs:
            ...
            # Strategy 1b: Try a broader query
        ...
        # Strategy 2: If still no docs, try key terms
        if not context_docs:
            ...
            for doc in context_docs:
                ...
                if doc_hash not in seen:
                    seen.add(doc_hash)
                    unique_docs.append(doc)
            context_docs = unique_docs[:20]

        # Strategy 3: If still no docs, try querying all sources (Fireflies if meeting query)
        if not context_docs:
        ...
        if not context_docs:
            ...
            if diag_docs:
                logging.warning(f"⚠️ Documents exist in database but query '{query}' didn't match.")
            else:
        ...
        purpose = (
            ...
            "passwords, system access, performance reviews, and Fireflies meeting transcripts. "
            "Prefer using internal documentation when it is relevant, but if it is missing or "
            "insufficient, answer from general HR and internal-ops best practices with clear, procedural steps."
        )
    ...
    def _extract_key_terms(self, query: str) -> list:
        ...
        hr_keywords = {
            ...
            "access": ["access", "permission", "permissions", "grant", "request access", "do not have access"],
    ...
    async def ingest_data(self):
        """
        Ingest data from Google Drive and Fireflies
        """
        logging.info("=" * 60)
