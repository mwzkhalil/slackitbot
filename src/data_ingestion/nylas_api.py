...
class NylasAPI:
    ...
    async def get_calendar_events(self, calendar_id: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            if calendar_id:
                events = self.client.events.list(calendar_id=calendar_id, limit=limit)
            else:
                # Get from primary calendar
                calendars = self.client.calendars.list()
                if calendars:
                    primary_cal = calendars[0]
                    events = self.client.events.list(calendar_id=primary_cal.id, limit=limit)
                else:
                    return []
            ...
            for event in events:
                meeting_events.append({
                    ...
                    "start_time": event.when.start_time if event.when else None,
